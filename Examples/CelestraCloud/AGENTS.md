...
Two versioned, in-repo files are the source of truth for how to work in this repository. **Read both before doing any work:**

- **`.claude/agent-notes.md`** — append-only log of Leo's corrections and standing always/never directives. Newest entries at the bottom, one line each.
- **`.claude/memory/MEMORY.md`** — index of this project's memory store: one markdown file per project-scoped fact (decisions, constraints, gotchas, context not derivable from the code or git history), one index line per file.

**Write to them proactively, without being asked:**

- When Leo makes a correction or gives an "always X" / "never Y" instruction, immediately append a line to `.claude/agent-notes.md`. When a directive supersedes an earlier one, update or remove the stale line instead of leaving both.
- When a fact worth remembering surfaces, add a memory file under `.claude/memory/` plus its one-line index entry. Update or delete memories that turn out to be wrong rather than leaving stale ones.

**Never store project-scoped memories in a native or global memory system** — not Claude Code's `~/.claude/projects/<project>/memory/`, not Cursor "memories", not any hosted knowledge store. They belong in this repo so they are versioned in git and visible to Leo, to teammates, and to every other agent working here. A native store may hold at most a single pointer to `.claude/memory/`.
