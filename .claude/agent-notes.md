...
- MistDemo is the live-verification oracle for CloudKit wire behavior: anything exercised via the MistDemo web interface or `test-public` / `test-private` integration phases counts as live-confirmed — do NOT re-flag those as "unproven" / "needs a live container" (including users/caller, shares resolve/accept, assets/rereference/ASSETID, ownerRecordName, zone payloads). Prefer MistDemo over lagging `openapi.yaml` hedges; the `WhatCloudKitGotWrong` / `WhatTheAIGotWrong` DocC articles' "Still open"/Limitations were refreshed against MistDemo on the talk branch.
