...
- Before writing any synthesis/analysis document, READ the existing `.claude/memory/` files and the `WhatCloudKitGotWrong` / `WhatTheAIGotWrong` DocC articles first — they are the prior art and the new work must cross-reference (corroborate/sharpen/contradict) them rather than restate or duplicate them.
- MistDemo is the live-verification oracle for CloudKit wire behavior: anything exercised via the MistDemo web interface or `test-public` / `test-private` integration phases counts as live-confirmed — do NOT re-flag those as "unproven" / "needs a live container" (including users/caller, shares resolve/accept, assets/rereference/ASSETID, ownerRecordName, zone payloads). Prefer MistDemo over lagging `openapi.yaml` hedges or archaeology `_raw/` "never live-confirmed" notes; the `WhatCloudKitGotWrong` / `WhatTheAIGotWrong` DocC articles' "Still open"/Limitations were refreshed against MistDemo on the talk branch.
- The 2026-09-08 removal of the top-level `docs/` tree (folded into README + the DocC catalog) needed NO archive tag — Leo's call; the squash-merge preservation rule means *ask*, not *always tag*.
