...
extension WebAuthTokenManagerTests {
  ...
  public struct ValidationCredentialTests {
    ...
    public func hasCredentialsWithInvalidTokens() async {
      let manager = WebAuthTokenManager(
    ...
    public func getCurrentCredentialsWithInvalidTokens() async throws {
      let manager = WebAuthTokenManager(
        apiToken: Self.invalidAPIToken,
        webAuthToken: Self.shortWebAuthToken
      )
